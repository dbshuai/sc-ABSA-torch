...
class LocationEncoding(nn.Module):
    ...
    def forward(self, x, pos_inx):
        ...
        weight = self.weight_matrix(pos_inx, batch_size, seq_len).to(x.device)
        x = weight.unsqueeze(2) * x
        return x

    def weight_matrix(self, pos_inx, batch_size, seq_len):
        pos_inx = pos_inx.cpu().numpy()
        weight = [[] for _ in range(batch_size)]
...
class AlignmentMatrix(nn.Module):
    def __init__(self,hidden_dim):
        ...
        self.w_u = nn.Parameter(torch.Tensor(6*hidden_dim, 1))

    def forward(self, batch_size, ctx, asp):
        ctx_len = ctx.size(1)
        asp_len = asp.size(1)
        alignment_mat = torch.zeros(batch_size, ctx_len, asp_len).to(ctx.device)
...
class MGAN(nn.Module):
    def __init__(self, embedding_matrix,hidden_dim,polarities_dim):
        """
        :param embedding_matrix: embedding 矩阵
        :param hidden_dim: 隐层维度
        :param polarities_dim: 类别数目
        """
        super(MGAN, self).__init__()
        self.embed_dim = embedding_matrix.size(-1)
        self.hidden_dim = hidden_dim
        self.embed = nn.Embedding.from_pretrained(torch.tensor(embedding_matrix, dtype=torch.float))
        self.ctx_lstm = DynamicRNN(self.embed_dim,hidden_dim, num_layers=1, batch_first=True, bidirectional=True)
        self.asp_lstm = DynamicRNN(self.embed_dim, hidden_dim, num_layers=1, batch_first=True, bidirectional=True)
        self.location = LocationEncoding()
        self.w_a2c = nn.Parameter(torch.Tensor(2*hidden_dim, 2*hidden_dim))
        self.w_c2a = nn.Parameter(torch.Tensor(2*hidden_dim, 2*hidden_dim))
        self.alignment = AlignmentMatrix(hidden_dim)
        self.dense = nn.Linear(8*hidden_dim, polarities_dim)

    def forward(self, text_raw_indices,aspect_indices,text_left_indices):
